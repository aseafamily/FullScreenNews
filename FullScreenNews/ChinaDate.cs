...
namespace FullScreenNews
{
    public static class ChinaDate
    {
        ...
        private static Dictionary<string, string> gHoliday = new Dictionary<string, string>();
        private static Dictionary<string, string> nHoliday = new Dictionary<string, string>();
        ...
        public static string GetYear(DateTime dt)
        {
            ...
            string str = string.Format("{1}{2}年 [{0}]", yearSX[yDZ], yearTG[yTG], yearDZ[yDZ]);
            return str;
        }
        ...
        public static string GetMonth(DateTime dt)
        {
            ...
            int leapMonth = china.GetLeapMonth(year, 0);
        ...
        {
            string strReturn = "";
            string key = dt.Month.ToString("00") + dt.Day.ToString("00");
            if (gHoliday.ContainsKey(key))
            {
                object g = gHoliday[key];
                if (g != null)
                {
        ...
        public static string GetChinaHoliday(DateTime dt)
        {
            ...
            else if (leapMonth != iMonth)
            {
                ...
                string key = iMonth.ToString("00") + iDay.ToString("00");
                if (nHoliday.ContainsKey(key))
                {
                    object n = nHoliday[key];
                    if (n != null)
                    {
                        if (strReturn == "")
                        {
