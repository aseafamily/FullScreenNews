...
{
    public class NationalWeatherProvider : BaseProvider, IWeatherProvider
    {
        public int TodayWeatherIconWidth { get; set; } = 55;

        public int ForcastWeatherIconWidth { get; set; } = 40;

        public NationalWeatherProvider(ILoggerFacade logger, IAppConfigurationLoader appConfigurationLoader)
            : base(logger, appConfigurationLoader)
        {
            logger.LogType<NationalWeatherProvider>();
        }
        ...
        public async Task<List<WeatherResult>> GetWeather()
        {
            ...
            Logger.Log("NationalWeather Source: " + url, Category.Info, Priority.Low);
